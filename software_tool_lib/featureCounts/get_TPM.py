...
import click
from pybioinformatic import get_TPM, Displayer
file_content = """
...
def main(featureCounts_result_file: TextIOWrapper, out_file_prefix: str = 'TPM', min_value: float = None) -> None:
    """
    ...
    if featureCounts_result_file.name == '<stdin>':
        ...
        df = read_table(featureCounts_result_file, header=1, index_col=0).iloc[:, 4:]
...
@click.option('-i', '--gene_exp', 'gene_exp_file',
              metavar='<exp file>', type=click.File('r'), required=True,
...
@click.option('-o', '--output_prefix', 'output_prefix',
              metavar='<str>', default='TPM', show_default=True,
